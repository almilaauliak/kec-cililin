...
class KecamatanController extends Controller
{
    ...
    public function dataHalamanAdmin(Request $request, $page)
    {
        dd('is Ajax: ' . ($request->ajax()? 'yes' :'no'));
        if ($request->ajax()) {

            // Mengambil semua data dari tabel 'halaman' di mana 'page' sama dengan $page
            $halamans = Halaman::where('page', $page)
                ->select('id', 'page', 'kategori', 'judul', 'desc') // Pilih kolom yang ingin ditampilkan
                ->get();

            // Mengembalikan data dalam format DataTables
            dd($halamans);
            return response()->json(['halamans' => $halamans]);
            //  return DataTables::of($halamans)->make(true);
        }

        // Jika bukan request AJAX, kembalikan view
        dd("ini bukan ajax");
        return view('halamanAdmin.halamanAdmin', compact('page'));
    }
    public function halamanAdmin(Request $request, $page)
    {
